...
export const pricingBenchmarks = mysqlTable("pricing_benchmarks", {
  ...
  /** Display and comparison unit; legacy rows default to acres until reviewed. */
  unit: mysqlEnum("unit", ["acre", "linear_foot", "hour", "load", "stump", "flat"]).notNull().default("acre"),
  /** Public source URLs supplied for owner review; never treated as verified automatically. */
  sourceUrls: text("sourceUrls"),
  /** Approved benchmarks require a human review; legacy rows remain explicitly unverified. */
  evidenceStatus: mysqlEnum("evidenceStatus", ["legacy_unverified", "owner_approved"]).notNull().default("legacy_unverified"),
...
/**
 * Research suggestions generated by the pricing agent. They are deliberately
 * separate from active benchmarks so a model response cannot alter pricing
 * controls until the owner has reviewed the unit, source links, and range.
 */
export const pricingBenchmarkCandidates = mysqlTable("pricing_benchmark_candidates", {
  id: int("id").primaryKey().autoincrement(),
  serviceType: varchar("serviceType", { length: 100 }).notNull().unique(),
  lowAmount: int("lowAmount").notNull(),
  midAmount: int("midAmount").notNull(),
  highAmount: int("highAmount").notNull(),
  unit: mysqlEnum("unit", ["acre", "linear_foot", "hour", "load", "stump", "flat"]).notNull(),
  region: varchar("region", { length: 200 }).notNull().default("Middle & West Tennessee"),
  researchSummary: text("researchSummary").notNull(),
  sourceUrls: text("sourceUrls"),
...
export type PricingBenchmarkCandidate = typeof pricingBenchmarkCandidates.$inferSelect;
...
/**
 * Immutable Stripe webhook processing ledger. eventId is unique so retries from
 * Stripe never repeat a successful payment state change, while failed attempts
 * remain visible for reconciliation and are safe to retry.
 */
