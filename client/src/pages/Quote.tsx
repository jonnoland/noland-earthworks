...
export default function QuotePage() {
  ...
  const addDroppedAttachments = (event: React.DragEvent<HTMLDivElement>) => {
    event.preventDefault();
    setIsAttachmentDropActive(false);
    if (!isUploadingAttachments) void addAttachments(event.dataTransfer.files);
  };
            ...
            {submitted ? (
              ...
              <form onSubmit={submit} noValidate aria-busy={submitRequest.isPending}>
                ...
                <div className="mt-5 border border-white/15 bg-white/[0.03] p-4">
                  ...
                  <div onDragEnter={(event) => { event.preventDefault(); if (!isUploadingAttachments) setIsAttachmentDropActive(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { event.preventDefault(); setIsAttachmentDropActive(false); }} onDrop={addDroppedAttachments} className={`mt-3 border border-dashed p-4 transition ${isAttachmentDropActive ? "border-[#E07B2A] bg-[#E07B2A]/10" : "border-white/25 bg-black/10"}`}>
                    <input id="site-visit-attachments" type="file" multiple accept="image/jpeg,image/png,image/webp,application/pdf,.doc,.docx" className="sr-only" disabled={isUploadingAttachments} onChange={(event) => { void addAttachments(event.target.files); event.currentTarget.value = ""; }} />
                    <div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-['Lato'] text-xs font-semibold text-white/80">Drag and drop one or more files here</p><p id="site-visit-attachment-help" className="mt-1 font-['Lato'] text-[11px] text-white/50">Or choose files from your device. You can select multiple files at once.</p></div><label htmlFor="site-visit-attachments" className="inline-flex min-h-10 cursor-pointer items-center border border-[#E07B2A]/60 px-3 font-['Oswald'] text-[11px] font-semibold uppercase tracking-[0.1em] text-[#f0ede6] transition hover:bg-[#E07B2A]/15">{isUploadingAttachments ? <><Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" />Uploading…</> : "Choose files"}</label></div>
                  </div>
                ...
                {currentStep === 4 && <div className="space-y-4"><div><p className="font-['Oswald'] text-xs font-semibold uppercase tracking-[0.14em] text-[#E07B2A]">Review your request</p><p className="mt-2 font-['Lato'] text-sm leading-6 text-white/65">Confirm the details below before sending your Site Visit Request. You can go back and edit any section.</p></div><div className="grid gap-3"><div className="border border-white/12 bg-black/10 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-['Oswald'] text-xs font-semibold uppercase tracking-[0.1em] text-white/65">Contact</p><p className="mt-2 font-['Lato'] text-sm text-white">{form.name}</p><p className="mt-1 font-['Lato'] text-xs text-white/60">{form.phone} · {form.email}</p></div><button type="button" onClick={() => setCurrentStep(1)} className="font-['Oswald'] text-[11px] font-semibold uppercase tracking-[0.08em] text-[#f6ad68] underline underline-offset-2">Edit</button></div></div><div className="border border-white/12 bg-black/10 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-['Oswald'] text-xs font-semibold uppercase tracking-[0.1em] text-white/65">Property</p><p className="mt-2 font-['Lato'] text-sm text-white">{form.street}</p><p className="mt-1 font-['Lato'] text-xs text-white/60">{form.city}, {form.state} {form.zip} · {form.county}</p><p className="mt-1 font-['Lato'] text-xs text-white/60">{form.service} · {form.acreage} acres{selectedParcel?.parcelId ? ` · Parcel ${selectedParcel.parcelId}` : ""}</p></div><button type="button" onClick={() => setCurrentStep(2)} className="font-['Oswald'] text-[11px] font-semibold uppercase tracking-[0.08em] text-[#f6ad68] underline underline-offset-2">Edit</button></div></div><div className="border border-white/12 bg-black/10 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-['Oswald'] text-xs font-semibold uppercase tracking-[0.1em] text-white/65">Project</p><p className="mt-2 whitespace-pre-wrap font-['Lato'] text-sm leading-6 text-white/80">{form.message || "No additional project notes provided."}</p><p className="mt-2 font-['Lato'] text-xs text-white/60">Preferred contact: {form.preferredContact}{form.timing ? ` · Timing: ${form.timing}` : ""} · {attachments.length} attachment{attachments.length === 1 ? "" : "s"}</p></div><button type="button" onClick={() => setCurrentStep(3)} className="font-['Oswald'] text-[11px] font-semibold uppercase tracking-[0.08em] text-[#f6ad68] underline underline-offset-2">Edit</button></div></div></div></div>}
                <div className="mt-7 flex items-center justify-between gap-3 border-t border-white/10 pt-5"><button type="button" onClick={() => setCurrentStep((step) => Math.max(1, step - 1))} disabled={currentStep === 1} className="min-h-11 border border-white/20 px-4 font-['Oswald'] text-xs font-semibold uppercase tracking-[0.1em] text-white disabled:cursor-not-allowed disabled:opacity-30">Back</button>{currentStep < 4 ? <button type="button" onClick={nextStep} className="min-h-11 bg-[#E07B2A] px-5 font-['Oswald'] text-xs font-semibold uppercase tracking-[0.1em] text-white transition hover:bg-[#f28c35]">{currentStep === 3 ? "Review request" : "Continue"}</button> : <button type="submit" disabled={submitRequest.isPending || isUploadingAttachments} className="inline-flex min-h-11 items-center justify-center gap-2 bg-[#E07B2A] px-5 font-['Oswald'] text-xs font-semibold uppercase tracking-[0.1em] text-white transition hover:bg-[#f28c35] disabled:cursor-not-allowed disabled:opacity-60">{submitRequest.isPending ? <><Loader2 className="h-4 w-4 animate-spin" /> Sending your request…</> : isUploadingAttachments ? <><Loader2 className="h-4 w-4 animate-spin" /> Uploading attachments…</> : "Request a Site Visit"}</button>}</div>
