            ...
            {submitted ? (
              ...
              <form onSubmit={submit}>
                ...
                <div className="mt-5 grid gap-5 sm:grid-cols-2">
                  ...
                  <label className="font-['Oswald'] text-xs uppercase tracking-[0.12em] text-white/65">County *<select name="county" required value={form.county} onChange={(e) => { update("county", e.target.value); setAddressAreaNote(""); }} onBlur={() => validateField("county")} aria-invalid={Boolean(errors.county)} className={fieldClassName(Boolean(errors.county))}><option value="" className="bg-[#191919]">Select a service-area county</option>{SERVICE_AREA_COUNTIES.map((county) => <option key={county} value={county} className="bg-[#191919]">{county}</option>)}</select>{errors.county ? <span className="mt-1 block font-['Lato'] normal-case tracking-normal text-red-300">{errors.county}</span> : <span className="mt-1 block font-['Lato'] normal-case tracking-normal text-white/40">Required. We currently schedule site visits in the listed Middle and West Tennessee counties.</span>}</label>
                </div>
                  ...
                  <div className="relative">
                    <div className="flex items-end justify-between gap-3">
                      <label className="min-w-0 flex-1 font-['Oswald'] text-xs uppercase tracking-[0.12em] text-white/65">
                        ...
                        <input ref={addressFieldRef} name="street" autoComplete="street-address" value={form.street} onFocus={() => setAddressFocused(true)} onBlur={() => window.setTimeout(() => setAddressFocused(false), 160)} onChange={(e) => { update("street", e.target.value); setSelectedPlaceId(""); setAppliedPlaceId(""); setAddressAreaNote(""); setOutOfAreaCounty(""); }} className={fieldClassName()} placeholder="Start typing the property address" />
                        <span className="mt-1 block font-['Lato'] normal-case tracking-normal text-white/40">Choose a suggested address to fill County, City, and ZIP when available.</span>
                      </label>
                ...
                <div className="mt-5 grid gap-5 sm:grid-cols-[1fr_0.55fr_0.45fr]">
                  ...
                  <label className="font-['Oswald'] text-xs uppercase tracking-[0.12em] text-white/65">ZIP <span className="text-white/40">(from address)</span><input name="zip" autoComplete="postal-code" inputMode="numeric" pattern="[0-9]{5}(-[0-9]{4})?" title="Enter a 5-digit ZIP code" value={form.zip} onChange={(e) => update("zip", e.target.value)} className={fieldClassName()} placeholder="ZIP" /></label>
                  <label className="font-['Oswald'] text-xs uppercase tracking-[0.12em] text-white/65">Approx. size <span className="text-white/40">(optional)</span><input name="acreage" inputMode="decimal" pattern="[0-9]+([.][0-9]+)?" title="Enter acreage as a number" value={form.acreage} onChange={(e) => update("acreage", e.target.value)} className={fieldClassName()} placeholder="Acres" /></label>
                </div>
