...
import {
  ...
  ImageIcon, RefreshCw, CheckCircle2, AlertCircle, ChevronDown,
} from "lucide-react";
...
export default function Ads() {
  ...
  async function handleSaveAndPost(target: "facebook" | "instagram" | "both") {
    if (!generated) return;
    ...
    if (target === "instagram" || target === "both") {
      if (!generated.imageUrl) {
        toast.error("Instagram requires an image. Generate with image enabled.");
        return;
      }
      igMutation.mutate({
        postId,
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-8">
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-5">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            {/* Image toggle */}
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-foreground">Include image</label>
              <button
          ...
          <div className="bg-card border border-border rounded-xl p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                ...
                {generated.imageUrl ? (
                  ...
                  <div className="w-full aspect-square bg-secondary rounded-xl border border-border flex flex-col items-center justify-center gap-2 text-muted-foreground">
                    <ImageIcon size={32} className="opacity-30" />
                    <p className="text-xs">No image generated</p>
                    <p className="text-xs opacity-60">Enable "Include image" and regenerate</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 pt-2 border-t border-border">
              {(platform === "facebook" || platform === "both") && (
                <Button
                  onClick={() => handleSaveAndPost("facebook")}
                  disabled={isPosting}
                  className="gap-2 bg-[#1877F2] hover:bg-[#166FE5] text-white border-0"
                >
                  <Facebook size={14} />
                  {fbMutation.isPending ? "Posting..." : "Post to Facebook"}
                </Button>
              )}
              {(platform === "instagram" || platform === "both") && (
                <Button
                  onClick={() => handleSaveAndPost("instagram")}
                  disabled={isPosting || !generated.imageUrl}
                  className="gap-2 bg-gradient-to-r from-[#833AB4] via-[#FD1D1D] to-[#FCAF45] text-white border-0"
                  title={!generated.imageUrl ? "Instagram requires an image" : undefined}
                >
                  <Instagram size={14} />
                  {igMutation.isPending ? "Posting..." : "Post to Instagram"}
                </Button>
              )}
              {platform === "both" && (
                <Button
                  onClick={() => handleSaveAndPost("both")}
                  disabled={isPosting}
                  variant="outline"
                  className="gap-2"
                >
                  <Send size={14} />
                  {isPosting ? "Posting..." : "Post to Both"}
                </Button>
              )}
              <Button
                variant="ghost"
                ...
                disabled={saveMutation.isPending || !!savedPostId}
            ...
            {(fbMutation.isSuccess || igMutation.isSuccess) && (
              <div className="flex items-center gap-2 text-sm text-green-400 bg-green-400/10 border border-green-400/20 rounded-lg px-4 py-2">
                <CheckCircle2 size={14} />
                Posted successfully.
                {fbMutation.data?.url && (
                  <a href={fbMutation.data.url} target="_blank" rel="noopener noreferrer" className="ml-auto flex items-center gap-1 text-xs underline">
