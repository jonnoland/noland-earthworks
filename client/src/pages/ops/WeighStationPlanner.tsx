...
export default function WeighStationPlanner() {
  ...
  const handleParcelLookup = async () => {
    if (!parcelCounty) {
      toast.error("Select the Tennessee county before looking up a Parcel ID.");
      return;
    }
    if (!parcelId.trim()) {
      toast.error("Enter the Parcel ID from Tennessee Property Viewer.");
      return;
    }
    try {
      const result = await lookupParcel.mutateAsync({ county: parcelCounty, parcelId: parcelId.trim() });
      if (!result.matches.length) {
        setParcelMatches([]);
        toast.error("No matching parcel was found. Verify the county and Parcel ID.");
        return;
      }
      setParcelMatches(result.matches as ParcelDestinationMatch[]);
      toast.success(`${result.matches.length} parcel match${result.matches.length === 1 ? "" : "es"} found.`);
    } catch (error: any) {
      toast.error(error.message ?? "Parcel lookup failed.");
    }
  };
    ...
    <div className="flex flex-col h-full min-h-screen bg-[#121212] text-[#F0EDE6]">
      ...
      <div className="px-6 pt-6 pb-4 border-b border-white/10">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Job Route Planner</h1>
        </div>
        <p className="text-sm text-white/50">
          Plan job routes, use property addresses or Parcel IDs, and see DOT scale stations along the way. Scale locations are from OpenStreetMap; check PrePass or Trucker Path for live status.
        </p>
      </div>
        ...
        <div className="w-full lg:w-96 flex-shrink-0 flex flex-col border-r border-white/10 overflow-y-auto">
          ...
          <div className="p-5 space-y-3 border-b border-white/10">
            ...
            <details className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
              <summary className="flex cursor-pointer items-center gap-2 text-xs font-semibold text-white/75">
                <FileSearch className="h-4 w-4 text-amber-400" />
                Use a Tennessee Parcel ID as destination
              </summary>
                ...
                {parcelMatches.map((parcel) => (
                  <div key={`${parcel.county}-${parcel.parcelId}`} className="rounded border border-white/10 bg-black/20 p-2.5">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-3">
                      <button type="button" onClick={() => useParcelDestination(parcel)} className="text-[10px] font-semibold text-amber-300 underline underline-offset-2">Use as destination</button>
