...
describe("customer portal phased quote summary", () => {
  ...
  it("keeps standard and phase-scoped discounts as the final visible pricing item, including when equipment cost is included", () => {
    const summary = getQuotePortalPhaseSummary([
    ...
    expect(summary.approvedPhaseSections[0].lineItems.map((item) => item.description)).toEqual([
      "Phase 1",
      "Phase service",
      "Required Equipment & Project Costs",
      "Phase discount",
    ]);
  });
});
